  ...
  return (
    <header className="border-b border-gray-200 bg-white px-6">
      <div className="mx-auto flex max-w-5xl items-center justify-between">
        {/* Logo */}
        <Link href="/dashboard" className="py-4 text-xs font-bold uppercase tracking-widest text-gray-900 hover:text-gray-600 transition-colors shrink-0">
          Class of 2029
        </Link>
          ...
          {TABS.map((tab) => {
            ...
            return (
              <Link
                key={tab.href}
                href={tab.href}
                className={`px-4 py-4 text-xs font-semibold uppercase tracking-wide border-b-2 transition-colors ${
                  active
